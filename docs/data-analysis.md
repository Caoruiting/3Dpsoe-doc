...
## 2. 种群分布箱线图分析
本组图表以箱线图形式，间隔展示不同迭代轮次的种群指标分布，更清晰地呈现每轮个体的离散程度、分位位置和异常点分布。
...
该现象本质是**单目2D约束下的深度歧义问题**：低2D重投影误差并不必然对应最低的3D姿态误差，仅靠2D投影约束无法完全消除深度维度的不确定性。
如果两者相关性强，说明当前Fitness是优质的代理优化目标；如果相关性弱或不稳定，则验证了单视角2D约束存在天然的深度歧义局限。
